...
Simulum Screensaver Pack Unix README
------------------------------------
...
To Install:
  ...
  * Copy files to the right directories.
    ...
    3. For each screensaver, you will see two files, e.g.
       simulumscr and simulumscr-bin.  Copy or symbolically 
       link simulumscr and simulumscr-bin to SCREENSAVER_BIN
  * Edit ~/.xscreensaver and add an entry for the screensaver.  For
    example, for Simulum, add the following to the programs section:
      "Simulum" /full/path/to/simulumscr -root \n\
    (the simulumscr.bin, simulumscr.jar saverbeans-api.jar files 
  ...
  * Run xscreensaver-demo to test and select.  Look for 
    "Simulum". If it works, you should see a moving star field
...
To Run:
  * Go to screensaver settings - the new screensavers will appear there.
    For a basic example, look for Simulum.

Release Notes:
  * JDK 1.4 support is upcoming in a future release.

  * If it does not work but you get no error, try running simulum 
  ...
  * If "Simulum" does not appear, try restarting xscreensaver 
